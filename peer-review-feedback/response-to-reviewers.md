...
*We agree and clarified this limitation.* The rotational thromboelastometry and fibrinolytic features were part of the full profile used to construct the principal-component representation and memory matrix. They therefore affected the distribution from which synthetic profiles were drawn. Their median relative errors were comparable to those of the other feature groups, but we assessed them only statistically. The mechanistic model represented thrombin generation, not fibrinolysis or clot viscoelasticity. The reorganized final Discussion paragraph now explains the consequence: the analysis did not establish biological fidelity across the full profile. Mechanistic validation of these features would require models that represent those processes.
