...
*We appreciate the push toward translational concreteness.* We strengthened the framing of the downstream-utility experiment while bounding the claim. The revised Results stated that the practical value demonstrated in this study included mechanistic-model calibration without using the real cohort directly in that calibration, comparisons of subgroups containing only three or five patients, and factor-level hypotheses for testing in a larger study. We were equally explicit that these results did not replace additional patients or establish readiness for clinical deployment. We also added the original motivating application to the Introduction and Discussion: using synthetic profiles to support deep learning models that predict TGA responses from coagulation factors or estimate factor profiles compatible with an observed TGA response. We did not evaluate either predictive task here. The revised Discussion therefore presented them as intended uses that require testing on held-out or external real patients, and it explained that the inverse model should return a distribution or set of compatible profiles rather than claim a unique reconstruction. Per R1.11, we also stated that the 0.94× downstream ratio was consistent with variance reduction rather than evidence of new biological content.

**Changes:** Introduction (original motivating application) + Results (Downstream Utility) + Discussion (bounded forward and inverse modeling use cases). `\rtwo{...}`
