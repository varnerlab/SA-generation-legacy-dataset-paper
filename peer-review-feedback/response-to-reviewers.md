...
*Agreed.* We adopt more conservative wording throughout. The subgroup analysis is now described as a power-limited, descriptive Mann–Whitney non-detection diagnostic: `p > 0.05` means that this test did not detect a difference in that replicate and does not establish equivalence. We also qualify "clinically useful" as demonstrated only for the exploratory modeling use-cases tested, within a proof-of-concept framing.
