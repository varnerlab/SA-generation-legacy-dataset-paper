...
**[PLAN → PENDING E1]** *Excellent and central question — this is the crux of attributing the result to SA rather than to dimensionality reduction.* We ran a PCA-space ablation suite: PCA+GMM, PCA+KDE, PCA+k-NN interpolation, and PCA+Gaussian copula, all fit in the identical d=18 PCA subspace and decoded through the identical evaluation harness (marginal MRE, cross-visit covariance error, cloud overlap, mechanistic KS test, novelty/memorization). The key result is that each PCA-based baseline fails a *different* axis while sharing the same preprocessing: [interpolation memorizes / density methods inject spurious variance or collapse at n=23], whereas SA is the only method that simultaneously matches marginals, preserves cross-visit covariance, passes the mechanistic envelope, and remains novel. This isolates the SA energy landscape as the source of the joint-structure and mechanistic fidelity. Regarding (iv) diffusion: a score network cannot be trained at n=23; the k-NN interpolation baseline represents the interpolation family, and we note this explicitly.
...
**[PLAN → PENDING E1]** *A fair point.* The PCA-space ablation suite (R1.1) adds exactly this class of fairer, low-sample-appropriate baselines — density estimation, mixture models, copulas, and manifold interpolation — all operating in the same reduced space as SA rather than being handicapped by the full-dimensional or per-visit formulations. We report their results alongside MVN/CTGAN/TVAE and discuss why each remains outperformed on the joint-structure and mechanistic axes despite favorable preprocessing.
