...
amplification, and mechanistic consistency with an independent 58-ODE model of
the coagulation cascade. At every level, synthetic patients were statistically
and mechanistically indistinguishable from their real counterparts. A downstream
utility test further showed that a mechanistic model calibrated entirely on
synthetic patients predicted held-out real patient outcomes as well as one
...
particularly the mechanistic validation using an independent ODE model, provides
...
from very small subgroups enables hypothesis generation and power analyses that
