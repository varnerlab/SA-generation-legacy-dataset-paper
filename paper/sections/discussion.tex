...
complications such as PCOS, preeclampsia, and gestational thrombophilia are difficult
...
effective pattern count to $K_{\text{eff}} \approx 4.6$ (Supplementary Table~S2),
...
The closest published precedent for validated longitudinal synthetic generation
is the VAMBN-MT framework of K\"uhnel et al.~\citep{Kuhnel2024}, which extended
a Variational Autoencoder Modular Bayesian Network with an LSTM time-encoder to
better capture cross-visit dependencies on the DONALD nutritional cohort and on
the Alzheimer's Disease Neuroimaging Initiative (ADNI) cohort. Their study is instructive because it makes the regime contrast precise.
With $N{=}1{,}312$ DONALD participants and 33 longitudinal variables, their
setting sits in the data-rich regime where deep parametric models can be
trained at all, and even there the approach required expert-curated module
assignments, manually specified Bayesian-network edge constraints,
hyperparameter tuning of an LSTM-augmented HI-VAE per module, and 10{,}000
synthetic resamples drawn from the trained model to recover a published
age-trend regression. Even with all of this scaffolding the best variant left
the full cross-visit correlation matrix at roughly 70\% relative Frobenius
error, and downstream time trends weaker than the strongest signals in the real
data could not be recovered at any synthetic sample size. Our setting inverts
both axes of theirs. With $K{=}23$ patients across 216 features we operate two
orders of magnitude smaller and squarely in the $n < p$ regime where the
VAMBN-MT architecture cannot be fit at all, and the non-parametric,
geometry-preserving design of SA replaces hyperparameter-tuned modular
structure with PCA-derived linear geometry, replaces dataset-specific
dependency rules (graduation-status monotonicity, age-time arithmetic
identities, macronutrient summation) with a generic mechanistic-model
validation pattern that transfers to any domain with a calibrated ODE model,
and adds an inference-time conditional-generation lever in the form of
multiplicity weighting that VAMBN-MT does not provide. These are complementary
positions in the design space rather than a head-to-head comparison. VAMBN-MT
addresses the moderate-cohort regime where deep generative models are feasible
but expert-specified modular structure is needed to learn cross-visit
dependencies, while SA addresses the small-cohort regime where parametric
models cannot be fit and the geometry of the few real patients must itself
carry the generative signal.

Beyond these statistical and geometric considerations, the mechanistic validation
introduced an approach that applies
...
of interest (pharmacokinetic models in drug development, physiological models in
...
completely, producing median MREs of $\approx$19\%, an order of magnitude worse than
SA, across all epoch counts tested (300--3,000), consistent with known GAN mode
collapse behavior on small datasets. TVAE achieved comparable marginal fidelity to SA
at high epoch counts (median MRE 1.8\% at 3,000 epochs), but operated on per-visit
rows ($n{=}90$) rather than concatenated longitudinal profiles ($n{=}23$), meaning it
could not capture cross-visit covariance structure and had no mechanism for conditional
subgroup generation. These results confirmed that SA's advantage at small $n$ is practical. Standard deep generative methods either fail outright (CTGAN) or
achieve marginal-level fidelity without the longitudinal and conditional capabilities
that SA provides.

Several additional limitations warrant discussion. Our study used a single longitudinal
dataset of $K{=}23$ patients, and further evaluation on independent datasets and
...
extrinsic tenase $k_{\text{cat}}$ (Supplementary Table~S4). These deviations likely
