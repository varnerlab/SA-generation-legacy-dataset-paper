...
Developed~PE.}
\rtwo{Synthetic profiles could also support the development of deep learning models that
...
\rone{The average subgroup results did not show whether the conditioned cohorts preserved
clinically important distribution tails. In the descriptive bootstrap Mann--Whitney
diagnostic, four of 24
...
\rone{The data representation imposed a further limit. Concatenating visits allowed the
method to preserve cross-visit relationships, but it required every patient to have the same
assays at the same aligned visits. The current pipeline could not directly accept irregular
timing, missing assays or visits, or variable-length records. Imputation, padding, or a
sequence-aware extension could support such data, but each would add assumptions that require
validation in a small cohort.}
\rtwo{The PCA representation imposed a related limit because it captured only linear structure.
...
\rboth{Thus, the present results did not establish performance on irregular, incomplete, or
strongly nonlinear records.}
