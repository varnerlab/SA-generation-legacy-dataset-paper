...
worldwide~\citep{Abalos2013}, further perturb this balance in ways that remain
...
in a PCA-reduced linear subspace of the data rather than fitting a parametric
...
rather than as part of a coherent longitudinal trajectory. Here, we extend this
approach to generate complete longitudinal profiles and rigorously validate the
results.

Specifically, we asked three questions. First, can SA generate synthetic longitudinal
patients from a very small cohort ($K{=}23$ pregnant patients, 72 coagulation features
per visit, 3 visits) that reproduce not only individual feature distributions but also
the joint cross-visit covariance structure and the signatures of rare clinical
subgroups (PCOS, $n{=}3$; PE, $n{=}5$)? Second, are these synthetic patients
consistent with known coagulation biochemistry, as assessed by an independent ODE
model of the coagulation cascade~\citep{BrummelZiedins2012}? Third, can synthetic
data substitute for real data in a downstream computational task? To address these
questions, we developed a pipeline that concatenates each patient's multi-visit
profile into a single vector, applies PCA for dimensionality reduction, and uses
multiplicity-weighted Langevin dynamics on the Hopfield energy landscape to generate
new patients, with a direction-magnitude decomposition that preserves the natural
variance structure of continuous clinical measurements. We found that SA-generated
patients reproduced per-feature means with median error of 1.4\%, preserved
cross-visit correlations that MVN's rank-deficient covariance could not represent,
and maintained condition-specific signatures when amplifying rare subgroups. An
independent 58-species ODE model of the coagulation cascade could not distinguish
real from synthetic patients, and a mechanistic model calibrated entirely on
synthetic patients predicted held-out real patient outcomes as well as one calibrated
on real data.
