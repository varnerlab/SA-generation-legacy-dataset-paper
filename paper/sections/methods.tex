...
every component and $q_k = r_k/\sum_j r_j$ exactly. The target can therefore be sampled
exactly by ancestral sampling: first sampling $k$ from $q$ and then drawing
...
chain near a randomly selected stored pattern and retaining the endpoint after $T{=}2000$
steps. Because initialization and finite chain length could affect the output independently
of the target distribution, we evaluated them with a four-rung sampler ladder: the published
anchor-initialized endpoint scheme, sphere initialization, burn-in-controlled thinned
pooling, and Metropolis-adjusted sampling. Exact ancestral sampling from
Eq.~\eqref{eq:exact_mixture} served as a separate analytic reference rather than as a fifth
rung. The evaluated fidelity and membership-inference measures were similar across the four
rungs and the analytic reference, indicating that the published implementation reproduced
the evaluated properties of the target distribution at $\beta^*$ (Supplementary
...
logarithmic sweep of $\beta$. Specifically, $\beta^*$ was the grid point with the greatest
downward curvature, calculated as the most negative finite-difference second derivative with
respect to $\log\beta$ (Supplementary Methods). This procedure depended on the geometry of the
$K$ patterns in $d$ dimensions rather than on the meaning of their features. For this dataset
...
$\beta/\beta^*\in[0.1,3.0]$ showed that the selected operating point balanced novelty and
...
\rboth{For unit-normalized memories, the component probabilities satisfy
$q_k=r_k/\sum_j r_j$. We summarized their concentration with the participation ratio
$K_{\text{eff}}=1/\sum_k q_k^2=(\sum_k r_k)^2/\sum_k r_k^2$. This was the number of equally
weighted mixture components that would have the same probability concentration: equal weights
gave $K_{\text{eff}}=23$, whereas concentrating weight on three stored profiles moved it toward
three. The quantity described how broadly repeated draws were distributed across stored
profiles; it did not represent an independent clinical sample size. To place a target fraction
$f_{\text{target}}$ of mixture-component probability on a designated subset, we used
$\rho=f_{\text{target}}K_{\text{bg}}/[K_{\text{des}}(1-f_{\text{target}})]$, where
$K_{\text{des}}$ and $K_{\text{bg}}$ are the designated and background pattern counts.}
...
PCA-space norms; conditioning acted through the multiplicity-weighted direction sampler.}
...
\State \rboth{Select $\beta^*$ at the greatest downward curvature of attention entropy, including $\log\mathbf{r}$ when conditioned}
\For{$i = 1, \ldots, N$}
...
$r\in\{3,5,10\}$. In each cell, we constructed the stochastic-attention memory matrix from
the training cohort and estimated a Gaussian generator from the same cohort; each produced
$100$ profiles. Stochastic attention used the
...
size. We constructed the stochastic-attention memory matrix from each subset, generated
$100$ profiles, and evaluated
