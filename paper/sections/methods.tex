...
\end{equation}
Each stored patient is the center of one component, $\beta^{-1}$ is the common component
variance, and the multiplicity weights enter the mixture probabilities directly. Because the
memories used here are normalized to unit length, the exponential factor is shared by every
component and $q_k = r_k/\sum_j r_j$ exactly. The target can therefore be drawn from
ancestrally, by sampling $k$ from $q$ and then
$\vxi\sim\mathcal{N}(\mathbf{m}_k,\beta^{-1}\mathbf{I})$, with no Markov chain. The cohort
analyzed in this work was generated with the Langevin implementation given next, which is what
the reported results describe. The analytic sampler was added afterward as a reference, using
the identical normalization, magnitude and decoding pipeline
(Supplementary Section~\ref{ssec:sampling-correctness}).}
...
\rone{The synthetic cohort analyzed in this work was produced by initializing each
Langevin chain near a stored memory pattern and retaining the endpoint of a single
trajectory of $T{=}2000$ steps. Because that choice could in principle shape the
generator independently of the target distribution, we characterized it directly with
a mixing diagnostic and a four-rung sampler ladder that compares the published scheme
against sphere-initialized, burn-in-controlled, thinned and pooled Metropolis-adjusted
sampling, and against exact ancestral draws from Eq.~\eqref{eq:exact_mixture}. The
schemes agree closely in fidelity and in the memorization behavior relevant to privacy,
and the analytic reference agrees with them as well, indicating that the published
scheme faithfully represents the target law at $\beta^*$
...
\rboth{Put plainly, $\beta$ sets how sharply the sampler commits to one stored patient. When
$\beta$ is small the attention weights are nearly uniform, every stored patient contributes
about equally, and samples collapse toward the cohort average. When $\beta$ is large a single
patient dominates and samples reproduce that individual. The useful regime is the transition
between the two, where stored patients stop blurring into one average and begin to act as
distinct attractors, and $\beta^*$ is our estimate of where that transition occurs.}
...
designated subset size (strongly conditioned). \rboth{This answers a concrete question. If
attention is spread unevenly across the $23$ stored patients, how many patients is the
generator effectively drawing on? Equal weights give $23$, and weights concentrated on three
patients give a value near three. Reporting $K_{\text{eff}}$ alongside a conditioned cohort
therefore states how much independent material that cohort rests on, which is the quantity
that bounds what can be concluded from it.} For a target effective fraction
