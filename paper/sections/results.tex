...
\rtwo{To ask whether the sampler stays within the real cohort or reaches beyond it, we
tested convex-hull membership of the synthetic patients against the $23$ real patients in
the $d{=}18$ principal-component space (Supplementary Table~\ref{stab:hull-geometry}).
Every synthetic patient fell outside the hull. So did every real patient with respect to
the other $22$, and when the comparison was matched for severity by removing each point's
nearest vertex on both sides, so that real and synthetic patients faced equally depleted
hulls, the distance to the hull was not distinguishable between them. The hull criterion
did not separate synthetic patients from real ones at this sample size.}

\rtwo{The reason is geometric rather than statistical. With $23$ points in $18$ dimensions
the hull is nearly a simplex, reaching the patients' typical radius only along the $23$
vertex directions themselves and extending a small fraction of that distance in a generic
direction, so almost any point that does not sit on a stored pattern lies outside it. The
sampler compounds this because it normalizes the stored patterns to unit length and returns
each sample to the unit sphere, and a unit vector inside the convex hull of unit-norm
patterns must coincide with one of those patterns, which makes hull membership structurally
unavailable to this generator rather than merely uncommon. We therefore read the result as a
limit on what the hull criterion can resolve at $K{=}23$ rather than as evidence of runaway
extrapolation, and assessed the generated points against the plausibility criteria directly,
where most satisfied the biological constraints and per-feature mechanistic ratios stayed
inside the real cohort's percentile band for the large majority of patients, although the
strict criterion requiring every feature and visit to fall inside the band simultaneously
was met by fewer than half.}

\subsection{\rone{Re-identification Risk}}

\rone{Because the energy landscape places a basin at each stored patient, we asked how much
the generated cohort reveals about who was in it. For every synthetic patient we measured the
distance to the closest real record and compared it against the leave-one-out distance between
real patients, and we ran a nearest-neighbor membership-inference attack that retrains the
generator on a subset of the cohort and scores each real record by its proximity to the
resulting synthetic set (Supplementary Table~\ref{stab:membership}). Synthetic patients sat
slightly closer to the real cohort than real patients sat to one another, and the attack
separated training members from held-out patients with a mean AUC of $0.97$ across $40$ random
splits, reaching perfect separation on roughly a fifth of them. At the published operating
point the generator therefore reproduces its training cohort closely enough for membership to
be recovered.}

\rone{Decomposing the attack shows what it does and does not recover. Held-out patients sat at
essentially the generic distance between two unrelated patients from the synthetic cohort,
matching the leave-one-out distance among the real patients themselves, whereas training
members sat considerably closer. The attack
therefore recovers which patients were stored in the memory rather than information about
patients the generator never saw, and the separation follows from the same small-cohort
geometry that left the hull criterion uninformative. The mechanism is visible in the form of
the target itself: the distribution the energy induces is a mixture with one component centered
on each stored patient (Eq.~\eqref{eq:exact_mixture}), so every draw descends from a single
stored patient and carries proximity to that patient into the decoded profile. Nothing in the
construction averages membership across several patients. The attack also assumes
an adversary who already holds a patient's record and asks whether it was in the training
cohort, so it bounds membership disclosure rather than showing that patient-level values can be
reconstructed from the synthetic cohort alone. Whether the leak reflects the target
distribution or the sampler used to draw from it is taken up separately, where
Metropolis-adjusted sampling and a wide sweep of the inverse temperature both leave it
essentially unchanged, and exact ancestral draws from Eq.~\eqref{eq:exact_mixture}, which
involve no chain at all, reproduce it undiminished at an AUC of $0.98$
(Supplementary Table~\ref{stab:sampling-ladder}, Fig.~\ref{sfig:sampling-ladder}). The signal
belongs to the target distribution at this cohort size, not to the sampler.}
...
Developed~PE cohort (Table~\ref{stab:multiplicity}), so each cohort of $100$ generated
patients is a recombination of three and five real individuals respectively. Those cohorts
carry no more information about the conditions than the underlying patients do, and any
downstream analysis treating them as $100$ independent observations will understate
uncertainty. What conditioning provides is a denser sample of the distribution implied by
those few patients, which supports hypothesis generation and power calculation rather than
additional evidence about the conditions themselves.}
...
population-level plausibility. \rboth{A mechanistic model of the coagulation cascade,
structurally independent of the generator but calibrated on the same cohort, placed
SA-generated patients within the same biologically plausible envelope as real patients, and
could not distinguish them under either experimental condition.}
...
the synthetic training set derives from the same $23$ patients, so the modest advantage is a
variance-reduction effect rather than evidence of new biological content. Fitting five rate
constants against $100$ samples drawn from a distribution estimated on $23$ patients smooths
the objective and dilutes the influence of any single patient, which would be expected of any
generator producing a denser sample of the same distribution. The result establishes that the
synthetic cohort is a usable substitute for calibration, not a better one.}

\rtwo{Read alongside the conditioned cohorts, this bounds what the method offers in practice.
It enables analyses that the original sample size forecloses, calibrating a mechanistic model
without spending the real cohort on training and comparing subgroups holding three and five
patients, and it points to factor-level differences a larger study could test. It does not
substitute for additional patients, and none of these results speak to clinical deployment,
which would require prospective validation against real clinical decisions.}
