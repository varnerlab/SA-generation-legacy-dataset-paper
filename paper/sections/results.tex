...
Table~\ref{stab:pca-ablation}). The comparison is instructive precisely because no single
method wins. Two baselines each failed a distinct axis: $k$-nearest-neighbor interpolation
collapsed novelty (median $0.05$, essentially memorizing the training patients), and the
kernel-density estimator degraded both marginal fidelity ($1.9\%$ median relative error)
and mechanistic plausibility ($0.82$ cloud overlap). The mixture and copula baselines,
however, were competitive with SA on marginal error ($1.1\%$ and $1.3\%$ against SA's
$1.2\%$), mechanistic overlap ($0.92$ and $0.93$ against $0.90$), and novelty. We read this
honestly: with every method sharing the same PCA subspace, much of the marginal and
mechanistic fidelity is supplied by that subspace rather than by SA specifically, and the
empirical cross-visit correlation error actually favored the baselines that stayed nearer
the $23$ real patients, since reproducing the sample correlation matrix is easiest by not
straying from it. Covariance \emph{generalization}, as distinct from reproduction of the
sample, is therefore established not here but on the simulation benchmark with a known
population covariance (Fig.~\ref{fig:sim-recovery}A). What SA adds over the competitive
subspace baselines is the energy-landscape formulation itself, which supplies the
closed-form operating point and the inference-time multiplicity steering used below, at a
fidelity that matches the strongest of them.}
