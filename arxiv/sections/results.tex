...
way, the Mann--Whitney test detected no difference in distance ($p{=}0.07$). The hull criterion did
...
plausibility criteria instead. Fifty-four of 100 satisfied all implemented biological
constraints. The 46 failures had at least one negative decoded hormone concentration:
19 patients for estradiol and 37 for progesterone, with 10 overlapping. Most negative
measurements were close to zero relative to the real-cohort scale, but the tails reached
$-2109.8$ for estradiol ($-0.42$ real-cohort SD below zero) and $-75.8$ for progesterone
($-1.24$ SD). These values arise because Gaussian sampling in PCA space followed by linear
decoding has unbounded support; we did not clip them because clipping would alter the
validated covariance structure. The unconstrained concatenated MVN baseline showed a
comparable failure rate (58 negative hormone measurements across 43 of 100 patients, versus
57 measurements across 46 of 100 for SA), so this is a limitation of unconstrained
continuous generation rather than a uniquely SA-specific failure. It remains a biological
invalidity in both methods. Per-feature mechanistic ratios stayed inside the real
cohort's percentile band for 83--92\% of patients. Forty-one of 100 met the strict
all-features-and-visits mechanistic criterion, and 24 of 100 met both that criterion and all
...
that replicate. This power-limited fraction is descriptive and does not establish
...
diagnostic is not used to make simultaneous inferential claims; the per-pair values and the
90\% threshold are descriptive reporting summaries.} PCA projections of the conditioned cohorts are provided in
...
That supports exploratory hypothesis generation and simulation-based power calculations
whose uncertainty remains anchored to the original patients. It is not additional evidence
about the conditions themselves, and it is not a substitute for recruiting patients.}
...
SA-generated patients within the same model-predicted envelope as real patients under both
experimental conditions. The model was calibrated on this same cohort, so this is a
