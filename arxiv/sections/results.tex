...
($r{=}3,5,10$), we fit stochastic attention and a sample-covariance Gaussian generator to the
same training cohort, generated $100$ profiles from each, and compared each generated
...
the clinical data by drawing ten random subsets at each of $n{=}20,15,10,$ and~$8$, fitting
stochastic attention to each subset, and scoring each generated cohort against the same full
$23$-patient reference. As $n$ fell from $20$ to~$8$, mean cross-visit correlation error rose
...
nonlinear target: a noisy S-curve embedded in $p{=}30$ or~$120$ dimensions. We fit both
generators to $n{=}23$ samples and measured both distance from the known curve and covariance
error as curvature increased. The off-manifold error of stochastic attention doubled
...
\rone{The amplification was bounded by how few patterns underlay it. At the target effective
...
represented a recombination of three and five real individuals. Those cohorts carried no more
information about the conditions than the underlying patients did. Any downstream analysis
that treated them as $100$ independent observations would understate uncertainty.
Conditioning provided a denser sample of the distribution implied by those few patients.
This supported exploratory hypothesis generation and simulation-based power calculations
whose uncertainty remained anchored to the original patients. It did not provide additional
evidence about the conditions themselves and was not a substitute for recruiting patients.}
...
\rone{The direction of that comparison was not interpreted as evidence of superiority. Every piece of information in
the synthetic training set derived from the same $23$ patients. The modest advantage was a
variance-reduction effect, not new biological content. Fitting five rate
constants against $100$ samples drawn from a distribution estimated on $23$ patients smoothed
the objective and diluted the influence of any single patient. Any generator producing a
denser sample of the same distribution would have done the same. The result established that the
synthetic cohort was a usable substitute for calibration, not a better one.}

\rtwo{Read alongside the conditioned cohorts, this bounded what the method offered in
practice. It enabled analyses that the original sample size foreclosed. We calibrated a
mechanistic model without spending the real cohort on training, and we compared subgroups
holding three and five patients. It also pointed to factor-level differences a larger study
could test. It did not substitute for additional patients. None of these results spoke to
