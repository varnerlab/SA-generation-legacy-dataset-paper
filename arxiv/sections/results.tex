...
stored pattern in angular distance (see novelty column of Table~\ref{stab:beta-sweep}). Second, we computed
...
features achieved $p > 0.05$ in $\geq$96\% of replicates.
Figure~\ref{sfig:mechanistic-tm-combined} shows the same scatter and ratio
analysis as the main-text comparison but under TF+TM conditions. There the
model systematically overcorrected the protein~C anticoagulant pathway,
producing peak and maximum rate predictions approximately 0.5$\times$ measured
values for both real and synthetic patients, yet this shared systematic bias
...
The calibration also generalized across pregnancy timepoints.
Figure~\ref{sfig:generalization} shows the predicted-to-measured ratios by visit.
Although fitted only on Visit~1 real patients, the median ratios at Visits~2
and~3 under TF-only conditions were 0.96$\times$ and 1.07$\times$, respectively,
confirming that the calibrated rate constants captured a stable
