...
geometry that left the hull criterion uninformative. At this cohort size the stored patterns
lie far enough apart in the standardized space that their basins do not overlap, so no region
of the landscape mixes several patients and averages membership away. The attack also assumes
...
distribution or the sampler used to draw from it is taken up separately, where an exact
Metropolis-adjusted sampler and a wide sweep of the inverse temperature both leave it
essentially unchanged (Supplementary Table~\ref{stab:sampling-ladder},
