...
- [ ] **Step 1: Extend the baseline paragraph with E1** (`results.tex:57-65`, after CTGAN/TVAE) — one paragraph citing the new supplement E1 table. **Framing (user decision, novelty–fidelity trade-off — NOT "SA wins all axes"):** with all methods sharing the identical d=18 PCA subspace, PCA+kNN fails novelty (memorizes; median novelty 0.05), PCA+KDE fails marginals + mechanistic plausibility (MRE 1.94%, overlap 0.823 below the plausibility floor), while PCA+GMM and PCA+Copula are fidelity-competitive on marginals and mechanistic overlap but generate less novel patients; **SA is the only method that combines full novelty (100% of samples, median 0.51) with competitive marginal (1.24%) and mechanistic (0.90) fidelity.** State honestly that the empirical cross-visit correlation Frobenius is *better* for the memorization-leaning baselines because staying close to the 23 reals reproduces the empirical correlation matrix for free — so the covariance-*generalization* point is made by SIM arm 1 (true population covariance, where memorization cannot cheat), which this paragraph must cross-reference. R1.1 takeaway: the shared PCA subspace supplies much of the fidelity; SA's distinctive contribution is principled novel generation at competitive fidelity. Numbers from `pca_ablation_results.csv`. `\rone`.
...
- E1 (REVISED per empirical result + user framing): E1 attributes the fidelity to the shared PCA subspace and identifies SA's distinctive contribution as **novelty at competitive fidelity**, NOT blanket dominance. kNN fails novelty; KDE fails marginals+mechanistic; GMM/Copula are fidelity-competitive but less novel; SA is the only method with 100% novelty AND competitive marginal+mechanistic fidelity. The empirical-covariance axis favors memorization-leaning baselines (expected); covariance *generalization* is demonstrated by SIM arm 1. (Success = the honest novelty–fidelity story is reported with real numbers and cross-referenced to SIM arm 1 — not the original "SA passes all four.")
